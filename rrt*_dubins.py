# Dubins-integrated RRT* based on uploaded rrt-star.py
# Will use dubins_path_planning and support non-holonomic forward-only motion
...
class Node(object):
    def __init__(self, x, y, yaw, parent=None, cost=0.0):
        self.x = x
        self.y = y
        self.yaw = yaw
        self.parent = parent
...
def dubins_steer(from_node, to_node):
    ...
    for (x, y) in zip(px, py):
        if collides((x, y)):
            return None
    new_node = Node(px[-1], py[-1], pyaw[-1], parent=from_node)
    new_node.path_x = px
    new_node.path_y = py
    new_node.path_yaw = pyaw
    new_node.cost = from_node.cost + sum(clen) if isinstance(clen, list) else from_node.cost + clen
    return new_node
...
def draw_path(goal):
    ...
    while node.parent is not None:
        for i in range(1, len(node.path_x)):
            pygame.draw.line(screen, red,  # red
                             (node.path_x[i - 1], node.path_y[i - 1]),
                             (node.path_x[i], node.path_y[i]), 4)  # thicker line
        node = node.parent

def main():
    global FPS
    FPS = 1000  # Define FPS at the beginning to avoid scope issues
    screen.fill(white)
    ...
    while selecting:
        for e in pygame.event.get():
            ...
            if e.type == pygame.MOUSEBUTTONDOWN:
                x, y = e.pos
                if not start_node:
                    start_node = Node(x, y, np.deg2rad(0.0))
                    nodes.append(start_node)
                    pygame.draw.circle(screen, red, (x, y), GOAL_RADIUS)
                elif not goal_node:
                    goal_node = Node(x, y, np.deg2rad(90.0))
    ...
    while True:
        ...
        if new_node:
            ...
            if dist((new_node.x, new_node.y), (goal_node.x, goal_node.y)) < GOAL_RADIUS:
                end_time = time.time()
                goal_node.parent = new_node
                goal_node.cost = new_node.cost + dist((new_node.x, new_node.y), (goal_node.x, goal_node.y))
                print("✅ Goal reached")
                print(f"⏱ Time taken: {end_time - start_time:.2f} sec")
                print(f"📏 Path distance: {goal_node.cost:.2f} units")
                print(f"🌲 Tree depth: {len(nodes)}")
    ...
    if not path_found:
        print("❌ Goal not reached within max nodes")

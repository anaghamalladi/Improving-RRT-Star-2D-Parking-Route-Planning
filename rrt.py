    ...
    while True:
        if currentState == 'init':
            ...
            while currNode.parent is not None:
                total_distance += dist(currNode.point, currNode.parent.point)
                pygame.draw.line(screen, red, currNode.point, currNode.parent.point, 3)
                currNode = currNode.parent
                turns += 1
            end_time = time.time()
            print(f"\n✅ Shortest Distance: {total_distance:.2f} units")
            ...
            if count < NUMNODES:
                ...
                if dist(parentNode.point, newnode) <= GOAL_RADIUS:
                    continue
                for p in nodes:
                    if dist(p.point, newnode) + dist(p.point, p.parent.point if p.parent else p.point) <= dist(parentNode.point, newnode):
                ...
                if point_circle_collision(newnode, goalPoint.point, GOAL_RADIUS):
                    goalNode = Node(goalPoint.point, parentNode)

...
def init_obstacles_parkingLot1(configNum):  #initialized the obstacle
    ...
    parking_space_height = 30  # Distance between two rows
...
def init_obstacles_parallelParking(configNum):
    ...
    Treat houses as obstacles.
    The car should now park itself in an empty parallel parking spot using the Reeds-Shepp path algorithm.
    """
    global obs, empty_spaces, car_position
    ...
    # Draw houses on both sides of the road and treat them as obstacles
    house_width = 60
    house_height = 80
    house_gap = 30
    for i in range(num_spaces):
    ...
    # Make the car park in the closest empty spot using Reeds-Shepp path
    if empty_spaces:
        ...
        px, py, pyaw, mode, clen = reeds_shepp_path_planning.reeds_shepp_path_planning(
            car_start_x, car_start_y, 0, car_goal_x, car_goal_y, np.deg2rad(90), 0.1, 2.0
        )

        # Draw car movement path
        if px is not None:
            for i in range(1, len(px)):
                pygame.draw.line(screen, blue, (px[i - 1], py[i - 1]), (px[i], py[i]), 2)

        pygame.draw.rect(screen, (0, 0, 255), (car_goal_x, car_goal_y, parking_space_length, parking_space_width))  # Draw car in blue
...
class RRT():
    ...
    def steer(self, rnd, nind):
        ...
        if abs(newNode.yaw - nearestNode.yaw) > np.deg2rad(60):  
            return None  # Ignore unnatural sharp turns
...
def main():
    ...
    #initialPoint = Node(62, 177, np.deg2rad(0.0))
    #goalPoint = Node(195, 157, np.deg2rad(0.0))
